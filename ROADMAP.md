...
- **Step 5 — in progress.** Decisions frozen (D11–D17). ai-arena ✓ (`v0.1.0`, PR #28). Next: Worldview Council (actual diff for review first), then StarLedger → StarLedger Template (via the parent's `README.template.md`) → portfolio / homebrew-tap licensing → bilingual filenames → forks. One repository per cycle: diff proposal → PR → CI → merge → release. Then step 6 extensions.
