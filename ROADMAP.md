...
- **Launch critical path (owner roadmap, 2026-09-24):** Deploy Readiness Audit (read-only proposal) → CI + test baseline → page states (custom 404 · In Preparation placeholder · Under Maintenance) → SEO metadata / robots / sitemap → Cloudflare Worker preview → `workers.dev` production test → `ccso.shsl.world` (+ `secc.studio` 301) → production smoke test → content (About, cases, Journey) → Writing → Topics → launch polish. Milestones M1 technically deployable → M2 pre-launch production → M3 public MVP → M4 full v1.
