...
| --- | --- | --- |
| **A — Original** | `portfolio`, `starledger`, `starledger-template`, `ai-pet-usage`, `opus-pack`, `tailscale-ai-egress`, `religion-council`, `ai-arena` | §2–§7 in full; §8 back-link after the portfolio launches |
...
| opus-pack | Claude Code plugin marketplace for agent discipline, reusable skills, executable gates and evaluation. |
...
- `applied-ai` is an umbrella only on Tier A repos that are Applied-AI evidence: `ai-arena`, `opus-pack`, `religion-council`. It is not a personal-brand tag.
- `governance`, `evaluation`, `security`, `privacy` and similar are added only where the repository demonstrates them (§7). Current calls: no `governance` on `opus-pack`; no `evaluation` on `ai-arena` (a comparison gallery, not a scored benchmark) — use `llm-comparison`; `security` on `tradingview-mcp` is supported by its README "Security boundary" section and CI-enforced tool allowlist.
...
| opus-pack | 8 | `applied-ai` `agent-skills` `ai-agents` `evaluation` `claude-code-plugin` `claude-code` `developer-tools` |
...
| opus-pack | `alpha-0.1.2` (decoupled) | `v0.1.16` (pre-release) |
...
| starledger · starledger-template · ai-pet-usage · opus-pack · religion-council | ACTIVE |
