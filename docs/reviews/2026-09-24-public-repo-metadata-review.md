...
## Post-review implementation decisions

*Addendum, 2026-09-24, after Steps 1–4 completed. Everything above this heading is the point-in-time review record and is unchanged. The decisions below were taken by the owner on the Step 5 proposal, relayed through the external-model review pass; the factual claims were re-verified before recording. Implementation state lives in issue #1 and `ROADMAP.md`.*

| # | Decision |
| --- | --- |
| D11 | **ai-arena:** MIT for the code (`LICENSE` holder: Eric So); a contribution/content policy in `CONTRIBUTING.md` with the revised rights wording — *by submitting non-code content, the contributor confirms that they have the necessary rights to submit it and grants the project a non-exclusive, worldwide, royalty-free licence to host, reproduce, format, display and redistribute the contribution as part of AI Arena and its repository; third-party and model-provider terms continue to apply* — an honest removal clause (*removal requests may be considered for the maintained site and current branch, but removal cannot be guaranteed from Git history, forks, caches or previously distributed copies*) and the confidentiality / personal-data confirmation, mirrored as two PR-template checkboxes. Rejected: "permanent display and redistribution rights" next to "removable on request" (contradictory). `v0.1.0` is a **normal release**, not a pre-release (`0.x` already states maturity; the site is live). `README.zh-TW.md` / `CONTRIBUTING.zh-TW.md` → `zh-Hant`. `Daily Workflow` stays as the quick-start section (the standard allows a different heading). |
| D12 | **religion-council (Worldview Council):** remove the v0.5 → v0.13.1 chronological narrative from the README (history lives in the existing `CHANGELOG.md`); keep a 2–3 paragraph **version-independent current-state** description so the next release does not touch the top fold; Highlights condensed **only from existing README / code / CHANGELOG evidence** — no new capability language (e.g. "RAG-ready retrieval contract" only if the repository shows that contract; an architecture direction is not a shipped highlight). |
| D13 | **starledger:** a **static exact-version release badge** (`v1.3.0-alpha.1`, linked to that Release), not a dynamic latest-release badge. The latest canonical release is a pre-release; shields' default `github/v/release` renders "no releases or repo not found" for this repository (observed 2026-09-24; `?include_prereleases` would be needed). Version representation is part of the release contract; a "README badge matches the latest Release" check is later release-process automation. |
| D14 | **homebrew-tap:** BSD-2-Clause for the tap; the README states explicitly that the tap licence ≠ the app licence (AI Pet Usage: AGPL-3.0) — one licence badge alone would mislead. Distribution glue stays minimal (no product sections). |
| D15 | **portfolio:** code MIT (`LICENSE` = unmodified MIT text); original written content, case studies, articles, images, graphics, logos and brand assets © Eric So, All Rights Reserved unless otherwise stated — scope listed in `NOTICE.md`, summarised in the README License section; `package.json` `"license": "MIT"` describes the package, not the site content. Rejected: CC BY-NC-ND for the content (complexity > benefit; NC / ND blur ordinary quotation, screenshots and excerpts). |
| D16 | **tailscale-ai-egress:** `README.zh-HK.md` → `README.zh-Hant.md` (the filename encodes the script convention); the display label stays **繁體中文（香港）** (the label names the variety actually used). CHANGELOG history is not rewritten; an Unreleased rename note is added. |
| D17 | **Forks (Tier B):** `jianying-headless` cannot be made private on its own — GitHub: *"You cannot change the visibility of a fork by itself"*; public forks stay public when the upstream goes private (docs re-read 2026-09-24). Reword the description to `Reference fork of mcncarl/jianying-headless — retained for reference; not maintained.` and archive; a separate private standalone copy only if actually needed — none is planned. `tradingview-mcp-atila-ref`: description cleanup → clear homepage → archive, in that order (archiving makes settings read-only). |
| D18 | **starledger-template README (2026-09-24, StarLedger cycle):** `README.template.md` is parent-maintained, so it carries **no** hard-coded child-release version badge — that would create a cross-repository release-state maintenance dependency; the template top fold is H1 `StarLedger Template` + value proposition + status + Apache-2.0 licence. Revisit only if the builder gains an explicit source for the template repository's release version. The parent README keeps `Phases and contracts` immediately after Quick start. |

**Step 5 execution order** (frozen with D11–D17): ai-arena licensing / content policy → ai-arena top fold + `v0.1.0` → Worldview Council → StarLedger → StarLedger Template regeneration (parent `README.template.md` → builder → template PR) → portfolio / homebrew-tap licensing → bilingual filenames → fork provenance. One repository per cycle — diff proposal → PR → CI → merge → release — never a cross-repository batch.
